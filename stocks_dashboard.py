...
col1.metric("Current Price", f"${information.get('currentPrice', 'N/A')}")
col2.metric("Market Cap", f"${information.get('marketCap', 0)/1e12:.2f}T")
col3.metric("52 Week High", f"${information.get('fiftyTwoWeekHigh', 'N/A')}")
...
if selection == 'Quarterly':
    ...
    net_income_chart = alt.Chart(quarterly_financials).mark_bar(color='green').encode(
        x='Quarter:O',
        y='Net Income'
    )
...
if selection == 'Annual':
    ...
    net_income_chart = alt.Chart(annual_financials).mark_bar(color='green').encode(
        x='Year:O',
        y='Net Income'
    )
    
st.altair_chart(revenue_chart, width="stretch")
st.altair_chart(net_income_chart, width="stretch")